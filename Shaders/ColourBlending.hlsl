...
{
    // min comparison cancels out any negative values generated by tonemapping non hdr pixels
    // I would have thought that min would leave the negative values remaining but max produces black values
    ...
    BaseColor = FastTonemap(BaseColor);
    BlendColor = FastTonemap(BlendColor);
    InterpolationColor = FastTonemap(InterpolationColor);
    
    half3 blendResult = ColourBlend(half4(BaseColor, 1.0h), half4(BlendColor, 1.0h), half4(InterpolationColor, 1.0h), Opacity, BlendType).xyz;
    
    // return max(blendResult, FastTonemapInvert(blendResult));
    return FastTonemapInvert(blendResult);
}


// if (Max3(BaseColor.r, BaseColor.g, BaseColor.b) > 1)
//     BaseColor = FastTonemap(BaseColor);

// if (Max3(BlendColor.r, BlendColor.g, BlendColor.b) > 1)
//     BlendColor = FastTonemap(BlendColor);

// if (Max3(InterpolationColor.r, InterpolationColor.g, InterpolationColor.b) > 1)
//     InterpolationColor = FastTonemap(InterpolationColor);

// half3 blendResult = ColourBlend(half4(BaseColor, 1.0h), half4(BlendColor, 1.0h), half4(InterpolationColor, 1.0h), Opacity, BlendType).xyz;

// if (Max3(blendResult.r, blendResult.g, blendResult.b) > 1)
//     blendResult = FastTonemapInvert(bl